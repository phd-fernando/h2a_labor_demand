...
#
# Script 31: Automation signals in CORN H-2A job orders, broken down by
# producing State. Runs Steps 1-3 (same methodology as scripts 18-30) both
# nationally and at the state level, then produces choropleth maps.
#
# Step 4 (new): Employer adoption heterogeneity — who is adopting, when,
# and does farm size or state predict early adoption?
#
...
#   Only JOs where corn is the PRIMARY crop are included — eliminates
#   double-counting from multi-crop JOs that mention corn incidentally.
#
# Corn: 12,395 JOs (vs 32,561 raw keyword hits — classifier removed ~62%
#   multi-crop contamination). Key producing states: KY, ND, AR, ID, MS, KS.
#   H-2A corn work is primarily detasseling, hand-harvesting seed corn, and
#   planting/scouting — mechanization targets precision planting, GPS guidance,
#   and automated detasseling machines.
###############################################################################
...
# 0. CROP PARAMETERS
# ==============================================================================
CROP_KEY     <- "corn"          # key in crop_patterns (00_crop_classifier.R)
CROP_NAME    <- "Corn"
CROP_WORDS   <- c("corn","maize","cornfield","detassel","detasseling",
                   "detasselled","tassel","tassels","silking","pollination")
SLUG         <- "corn"
...
auto_vocab <- tribble(
  ...
  "detassel.*machine|machine.*detassel",         "B",   "detasseling machine",
  "precision.plant|auto.*plant",                 "B",   "precision planting",
...
auto_cols_B <- c("auto_mechanical_harvest","auto_machine_harvest",
                 ...
                 "auto_detasseling_machine","auto_precision_planting")
...
ag_verb_pattern <- paste0(
  ...
  "detassel|tassel|scout|silk|pollinate"
)
...
verb_anchor <- paste0(
  ...
  "^record|^measur|^count|^flag|^sucker|^debud|^set|^detassel|^silk|^pollinate"
)
...
extract_verb_bigrams <- function(df_sent) {
  df_sent |>
    ...
    filter(!w1 %in% stop_words_all$word,
           ...
           str_detect(w2, "^[a-z]+$"),
           str_detect(w1, verb_anchor) | str_detect(w2, verb_anchor),
           !str_detect(w1, "robot|autonom|mechanic|machine|conveyor|sensor|gps|drone|detassel"),
           !str_detect(w2, "robot|autonom|mechanic|machine|conveyor|sensor|gps|drone|detassel")) |>
...
ggsave("processed/images/plot_corn_automation_national.png",
...
ggsave("processed/images/plot_corn_step4_adoption.png",
       p4_combined, width=16, height=7, dpi=180, bg="white")

# Maps
ggsave("processed/images/plot_corn_map1_mechanization_rate.png",
       p_map1, width=12, height=7, dpi=180, bg="white")
ggsave("processed/images/plot_corn_map2_hand_shift.png",
       p_map2, width=12, height=7, dpi=180, bg="white")
ggsave("processed/images/plot_corn_map3_adoption_timing.png",
       p_map3, width=12, height=7, dpi=180, bg="white")
ggsave("processed/images/plot_corn_map4_jo_density.png",
...
ggsave("processed/images/plot_corn_maps_composite.png",
...
saveRDS(
  ...
  "processed/text/automation_signals_corn.rds"
)
cat("Saved: automation_signals_corn.rds\nAll done.\n")
